...
clangpath = 'clang-3.5'
fakelib_parser = parserpath + '/utils/fake_libc_include/'
	...
	for lib in libs:
		if lib in src:
			beginInclude = src.find(lib, 0, len(src) )
		 	endInclude =  beginInclude + len(lb)
		 	src = src[:beginInclude] + '"' + lib + '"' + endInclude
		 	libsfound.append(lib)
		 	config4 = {'name':'cp', 'input':[fkd + lib, wd+]}
			cmd = Command(config4)
			cmd.execute()
...
def bmc_analyze(srcfile, rounds):



	temp = sourcefile+'.tmp'
	config = {'name': cbmcpath, 'input': ['--unwind', rounds, 
				'--no-unwinding-assertions', temp] }
	cmd = Command(config)
	cmd.execute()
	output = cmd.getOutput()
	print output
