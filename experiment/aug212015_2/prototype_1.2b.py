...
assertlib = '/home/heryxpc/Documents/soton/dissertation/benchmarks/SVCOMP2015/pthread-lit'
...
def pagai2pycparser(srcfile):
	...
	for lib in libs:
		if lib in src and not isInComments(lib, src):
			# Special case for assert library. Take it from specified path
			if lib == 'assert.h':
				assertd = os.path.abspath(assertlib)
				cpdep(lib, assertd, wd, libsfound)
				continue
