...
class TimeLog:
	...
	def seconds(self):
		self.count = self.endtime - self.initime
		return self.count
	...
	def displaySeconds(self):
		...
		print ('Elapsed time between start and end:'+ seconds)
...
def bmc_analyze(sourcefile, unwind):
	k_unwind = str(unwind)
	config = {'name': 'timeout', 'input': ['850', cbmcpath, '--unwind', 
				k_unwind, '--no-unwinding-assertions', sourcefile] }
	cmd = Command(config)
	cmd.execute()
	output = cmd.getOutput()
	if not output:
