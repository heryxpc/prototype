...
#define __VERIFIER_assume(cond) assert(!(cond))

/* Modified to handle non determinism in PIPS
void __VERIFIER_assert(int x) { if(!(x)) {ERROR: goto ERROR;}}
extern void __VERIFIER_assume(int); // <------------ SHALL WE INSTEAD USE THE ASSUME EXPRESION USING AN ASSERT FOR PIPS?
 ...
 *
 *  params:
 *    -l out, --backend cpachecker, --unwind 7, --rounds 7, -i ../../../prototype/test7/benchmarks/cseq/fib_bench_longer_false-unreach-call.c, 
 *
 *  modules:
...
void __cs_init_scalar(void *__cs_var, int __cs_size)
{
        if (__cs_size == (sizeof(int)))
                *((int *) __cs_var); //=  __VERIFIER_nondet_int(); <---------- REMOVED TO PROVIDE NONDETERMINISM
        else
        {
